...
| `FALLOW_AUDIT_CACHE_MAX_AGE_DAYS` | GC threshold in days for reusable audit base-snapshot caches; `0` disables the sweep. Wins over the `audit.cacheMaxAgeDays` config field; invalid values log a warning and fall back to config / default. Each sweep also reclaims abandoned cache entries minted under other repo identities (deleted or moved repos, other git worktrees) once they age past this threshold; entries whose recorded owner root still exists are skipped and stay governed by that repo's own setting, so its `0` (never sweep) cannot be defeated from the outside. `fallow audit-cache prune` applies the same policy on demand and reports per-entry decisions; its `--max-age-days` flag overrides this variable for one invocation. | `30` | `FALLOW_AUDIT_CACHE_MAX_AGE_DAYS=7` |
...
| Variable | Description | Default | Example |
| --- | --- | --- | --- |
| `FALLOW_COVERAGE` | Path to Istanbul coverage data (`coverage-final.json`) for accurate per-function CRAP scores. The `--coverage` flag overrides it; it wins over the `health.coverage` config field. Honored by the `health`, bare `fallow`, and `audit` CLI commands and by the MCP `audit` and `check_health` tools on both their typed route and their CLI fallback, where the explicit `coverage` parameter overrides it. | unset | `FALLOW_COVERAGE=coverage/coverage-final.json` |
| `FALLOW_COVERAGE_ROOT` | Absolute coverage-data path prefix for rebasing Istanbul paths in CI or containers. The `--coverage-root` flag overrides it; it wins over the `health.coverageRoot` config field. Honored by the `health`, bare `fallow`, and `audit` CLI commands and by the MCP `audit` and `check_health` tools on both their typed route and their CLI fallback, where the explicit `coverage_root` parameter overrides it. | unset | `FALLOW_COVERAGE_ROOT=/ci/workspace` |
...
| `FALLOW_TYPE_AWARE` | Enable or disable TypeScript semantic (type-aware) analysis for the run. Accepts `true`/`false`/`1`/`0`/`yes`/`no`/`on`/`off`; any other value is a hard error. Sits mid-chain in the precedence: the `--type-aware`/`--no-type-aware` CLI flags win over it, and it wins over the `audit.typeAware` config field, which wins over `typeAware.enabled`. | unset | `FALLOW_TYPE_AWARE=false` |
| `FALLOW_TYPE_AWARE_BIN` | Trusted executable override for the TypeScript semantic refinement sidecar used by `dead-code --type-aware`. Relative paths resolve from the caller's working directory before `--root` is applied. Fallow intentionally does not search project `node_modules` or `PATH`. The `fallow` npm launcher sets this itself when a version-matched `fallow-type-aware` npm package is installed, marking that wiring with the internal `FALLOW_TYPE_AWARE_BIN_SOURCE=npm-wrapper` variable so `type-aware status` reports `npm-wrapper` instead of `environment-override`. | sibling of active Fallow executable | `FALLOW_TYPE_AWARE_BIN=./node_modules/.bin/fallow-type-aware` |
...
| `FALLOW_BOT_LOGIN` | Explicitly allowlisted posting username for Fallow-owned finding roots and resolution replies. When set, ownership is narrowed to that exact username (an empty or whitespace-only value matches nobody). When unset, GitHub uses provider-native bot metadata, while GitLab also matches notes to the authenticated token owner returned by its current-user API. This is an authorship control, not a lifecycle or deduplication token. | unset | `FALLOW_BOT_LOGIN=fallow-bot` |
