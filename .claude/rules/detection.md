...
- **Angular external templates**: `@Component({ templateUrl: './app.html', styleUrl: './app.scss' })` decorators emit synthetic `SideEffect` imports from the component to its template/style files (preventing `unused-files` false positives). HTML template files are scanned for Angular syntax (`{{ }}`, `[prop]`, `(event)`, `[(ngModel)]`, `*ngIf`, `*ngFor`, `@if`, `@for`, `@switch`, pipes) and referenced identifiers are bridged to the component's class members via a sentinel in `member_accesses` (preventing `unused-class-members` false positives). `*ngFor` and `@for` bindings are tracked as locals.
