...
- `--coverage <PATH>` -- path to Istanbul-format coverage data (coverage-final.json from Jest, Vitest, c8, nyc) for exact per-function CRAP scores. Uses the canonical formula CC^2 * (1-cov/100)^3 + CC. Accepts a file or directory containing coverage-final.json. Also configurable via `FALLOW_COVERAGE` env var. Auto-detected from `coverage/coverage-final.json` or `.nyc_output/coverage-final.json` when present. CRAP model tiers: `"static_estimated"` (default, graph-based: directly test-referenced=85%, indirect=40%, untested=0%), `"istanbul"` (real coverage from `--coverage` or auto-detected file).
